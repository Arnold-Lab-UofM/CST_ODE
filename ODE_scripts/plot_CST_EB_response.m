...
% Feb, 19, 2021
% UPDATE: Jan 20, 2023 (new naming convention)
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

function plot_CST_EB_response(all_run_mat,newValueMat,dirName,pidx,sp_p,ep_p,time_post)
    
    % species colors and names
    ...
    for val_id = 1:size(all_run_mat,1)
        ...
        q = xline(sp_p,'--',{'Start Perturbation'});
        xe = ep_p + time_post;
        xs = sp_p - 24;
        if xs < 0
            xs = 0;
        end
        q2 = xline(ep_p,'--',{'Finish Perturbation'});
