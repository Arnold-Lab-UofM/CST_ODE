...
function  ode = lhs_ode_settings_GUI()
    dims = [1 50];
...
% Sample size.
ode.NR = str2double(run_info(1,:));

% Time span of the simulation.lhs_ode_settings_predator_prey_new
t_end = str2double(run_info(2,:)); % length of the simulations (in days)
...
ode.alpha = str2double(run_info(3,:));
...
% is not correct.
%
ode.odeModelHandle = str2func(run_info(4,:));
