...
function [dirName] = simulate_CST_EB_response_displace(ybase,ss_type,sel_nets,S,Jmat,perChange,sp_p,ep_p,time_post,plotTraj,vectorCell,pidx)
...
%     dirName = strcat(ss_typ_fix,'_',num2str(length(pidx)),'D_',pnmD,'_',date);
    ...
    all_warnings = NaN(size(newValueMat,1),size(sel_nets,1));
    NL = size(sel_nets,1);
    ...
    parfor val_id = 1:size(newValueMat,1)
        ...
        for net_id = 1:NL
            ...
            y0 = OGSS(id,:)+ ybase*sum(OGSS(id,:)); % MODIFIED FROM GIT (add displacement)
            ...
            [warnmsg1, ~] = lastwarn; 
            all_warnings(val_id,net_id) = warnmsg1 == "";
            lastwarn('')
